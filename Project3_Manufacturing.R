...
# Aim - Score = 1-(0.025/KS) ---> KS Score which should be greater than o.50
...
setwd("C:\\Users\\Monica\\Desktop\\Projects\\R Projects\\Project3")
...
## the final model is below - Linear Regression Model Building------------------------------------------------------------------------------------------------------------------
...
## score is very less(AUC is 0.669) so linear model cannot be fitted into the data very well -----------------------------------------------------------------
...
## obtaining auc on testing_data ---------------------------------------------------------------------------------------------------------------
auc(roc(as.numeric(testing_data$went_on_backorder),as.numeric(predict.score)))
## auc score is fine as it is ~ 0.9129 so this is the best fitted model here ------------------------------------------------------------------
...
## deciding the final cutoff on the basis of Maximum KS score ------------------------------------------------------------------------------
...
write.csv(final.test.prediction,"Monica_Desai_P3_part2.csv",row.names = F)
